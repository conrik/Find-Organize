...
namespace Find_And_Organize
{
    ...
    {
        public static bool ChkAtri(uint fileAtri)
        {
            const UInt32 FILE_ATTRIBUTE_RONLY = 0x1;
            const UInt32 FILE_ATTRIBUTE_SYSTEM = 0x4;
            const UInt32 FILE_ATTRIBUTE_HIDDEN = 0x2;
            if ((0 == (fileAtri & FILE_ATTRIBUTE_RONLY)) && 0 == (fileAtri & FILE_ATTRIBUTE_HIDDEN) && 0 == (fileAtri & FILE_ATTRIBUTE_SYSTEM))
                return true;
            else
                return false;
        }
                    ...
                    if (0 != (usn.FileAttributes & PInvokeWin32.FILE_ATTRIBUTE_DIRECTORY))
                    {
                        ...
                        if (!_directories.ContainsKey(usn.FileReferenceNumber))
                        {
                            _directories.Add(usn.FileReferenceNumber,
                                new FileNameAndParentFrn(usn.FileName, usn.ParentFileReferenceNumber, usn.FileAttributes));
                        }
                        //else
    ...
    public class FileNameAndParentFrn
    {
        ...
        #region Constructor
        public FileNameAndParentFrn(string name, UInt64 parentFrn, uint attributes)
        {
